    ...
    <div class="header bg-success pb-6">
      <div class="container-fluid">
        <div class="header-body">
          <!-- Card stats -->
          <h1 class="heading">User Gapoktan</h1>
        </div>
      </div>
    </div>
      ...
      <el-card v-loading="getLoader" style="margin-top: 60px">
        ...
        <vs-table striped>
          ...
          <template #tbody>
            <vs-tr
              :key="i"
              v-for="(tr, i) in $vs.getSearch(getUserGapoktans.data, search)"
              :data="tr"
            >
              <vs-td>
          ...
          <template #footer>
            <vs-row>
              <vs-col w="2">
                <small>Total : {{ getUserGapoktans.total }} Data</small>
              </vs-col>
              <vs-col w="10">
                <vs-pagination
                  v-model="page"
                  :length="Math.ceil(getUserGapoktans.total / table.max)"
                />
              </vs-col>
            </vs-row>
          </template>
...
export default {
  ...
  computed: {
    ...mapGetters("user", ["getUserGapoktans", "getLoader"])
  },
  methods: {
    logTest(data) {
      console.log(data);
    ...
    onSubmit(type = "store") {
      ...
      let url = "/tambah_gapoktan";
      if (type == "update") {
        url = `/ubah_gapoktan/${this.form.id}`;
      }
