...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("user/getAllUserAdmin", {})
  },
  computed: {
  ...
  methods: {
    ...
    onSubmit(type = "store") {
      ...
      console.log(this.form.aktif)
      formData.append("aktif", this.form.aktif == true ? 1 : 0);
      formData.append("foto", this.form.foto);
      formData.append("user_id", this.user_id);
      let url = "/tambah_admin";
      if (type == 'update') {
        url = `/ubah_admin/${this.form.id}`
      }
      this.$axios.post(url, formData).then(resp => {
        if (resp.data.success) {
          this.$notify.success({
            title: 'Success',
            message: `Berhasil ${type == 'store' ? 'Menambah' : 'Mengubah'} Admin`
          })
          this.resetForm()
